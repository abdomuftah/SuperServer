...
ensure_php_fpm(){
  ...
  for attempt in {1..12}; do
    ...
    if systemctl is-active --quiet "$unit" && [[ -n "$endpoint" ]] && endpoint_exists "$endpoint"; then
      printf '%s' "$endpoint"
      return 0
    fi
    sleep 1
...
available_php_versions(){
  ...
  for dir in /etc/php/*/fpm; do
    ...
    endpoint="$(php_fpm_listen_value "$version")"
    if systemctl is-active --quiet "php${version}-fpm" && [[ -n "$endpoint" ]] && endpoint_exists "$endpoint"; then
      printf '%s\n' "$version"
    fi
  done | sort -Vru
  shopt -u nullglob
...
case "${1:-}" in
  -h|--help) usage; exit 0 ;;
  --list-php) available_php_versions; exit 0 ;;
esac
...
install -m 0644 "$SHARED_ASSET_DIR/nginxExample.conf" "/etc/nginx/sites-available/$domain.conf"
...
if [[ "$endpoint" == /* ]]; then nginx_endpoint="unix:$endpoint"; else nginx_endpoint="$endpoint"; fi
