...
> [!TIP]
> **Wizard-first behavior:** SuperServer collects every choice before running `apt`, changing repositories, or installing services. After the final plan is approved, Composer, APT, npm and the remaining installers run non-interactively. Multi-select screens use terminal checklists: move with **↑/↓**, toggle with **Space**, and confirm with **Enter**.
